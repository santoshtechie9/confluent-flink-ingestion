#!/usr/bin/env python3
# Flink SQL (single-file) code generator driven entirely by STTM.xlsx.
# See header usage in previous message.
...
def extract_view_refs(cond: str) -> List[str]:
    # Grab referenced view names like hm_xxx_vw from a join condition.
    if not cond: return []
...
def build_views_section(cfg: pd.DataFrame, sttm: pd.DataFrame) -> str:
    ...
    for st in order:
        ...
        parts.append(f"""create view {view_name(vprefix, st, vsuffix)} ({alias_cols}) as
select
  {selects}
from {raw_table}
where
  json_value(cast({raw_col} as string), '{table_field}') = '{st}';""")
    return '\n\n'.join(parts) + '\n'
...
def build_sinks_section(cfg: pd.DataFrame, sttm: pd.DataFrame) -> str:
    ...
    for tt in order:
        ...
        parts.append(f"""drop table if exists {tt};
create table {tt} (
  {col_lines}
)
with ({withs});""")
...
def generate(sql_xlsx: Path, out_file: Path):
    ...
    parts = [
        build_views_section(cfg, sttm),
        build_sinks_section(cfg, sttm),
        build_inserts_section(cfg, sttm),
    ]
    out_file.parent.mkdir(parents=True, exist_ok=True)
    out_file.write_text('\n\n'.join(parts), encoding='utf-8')
    print('[done] generated FlinkSQL from STTM', out_file)
