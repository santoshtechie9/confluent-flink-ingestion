#!/usr/bin/env python3
# v4.3: Flink SQL generator from STTM with static validation + split YAMLs
# - Combined SQL file
# - Validation CSV (result first, then SQL; one line per statement with \n escaped)
# - views.yaml, sinks.yaml, inserts.yaml (each has a single "SQL queries: |" block)
import argparse
import re
...
def _resolve_sqlglot_dialect() -> str:
    # Pick a working sqlglot dialect, preferring ANSI-ish first.
    if not _HAS_SQLGLOT:
        return ''
...
def sink_with_options(cfg: pd.DataFrame, target: str) -> List[str]:
    ...
    for _, row in cfg.iterrows():
        ...
        if k.startswith('target_with.'):
            key = k[len('target_with.'):]; opts.append((key, v))
    for _, row in cfg.iterrows():
        k = safe(row.get('key')); v = safe(row.get('value'))
        if not k or not v: continue
        prefix = f'with.{target}.'
        if k.startswith(prefix):
            key = k[len(prefix):]; opts.append((key, v))
...
def _is_executable_statement(s: str) -> bool:
    if not s or not s.strip():
        return False
    t = s.lstrip()
    if t.startswith('--'):
        return False
    first = t.split(None, 1)[0].upper()
    return first in {'CREATE','DROP','INSERT','ALTER','EXPLAIN','EXECUTE','WITH','SELECT'}

def validate_statements(sql_text: str) -> List[Tuple[str,str]]:
    results: List[Tuple[str,str]] = []
    unwrapped = _unwrap_statement_set(sql_text)
    all_statements = _split_sql_statements(unwrapped)
    statements = [s for s in all_statements if _is_executable_statement(s)]
    if not _HAS_SQLGLOT:
    ...
    for stmt in statements:
        stmt_clean = _normalize_literal_escapes(stmt)
...
def write_yaml(path: Path, sql_block: str):
    data = 'SQL queries: |\n'
    for line in (sql_block or '').splitlines():
        data += f'  {line}\n'
...
def generate(sql_xlsx: Path, out_file: Path):
    ...
    with csv_path.open('w', encoding='utf-8') as f:
        ...
        for stmt, res in results:
            s = re.sub(r'\s+', ' ', stmt).replace('"', '""').replace('\n', ' ').replace('\r', ' ')
            r = res.replace('"', '""')
            f.write(f'"{r}","{s}"\n')
    base_dir = out_file.parent
    write_yaml(base_dir / 'views.yaml', views_sql.strip())
    write_yaml(base_dir / 'sinks.yaml', sinks_sql.strip())
    write_yaml(base_dir / 'inserts.yaml', inserts_sql.strip())
